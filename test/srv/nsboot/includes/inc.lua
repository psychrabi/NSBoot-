...
inc.getpid_nbd = function(t_path)
    local result
    if t_path ~= nil and inc.isFile(t_path) then
...
inc.scCheck = function()
    local result = true
    if inc.checkconf then
        if not inc.lsof("-t -i:" .. nsboot.cfg.dhcp.port) then
            result = false
        end
        if not inc.lsof("-t -i:" .. nsboot.cfg.tftp.port) then
            result = false
        end
        if not inc.lsof("-t -i:" .. nsboot.cfg.iscsi.port) then
            result = false
        end
    end
    return result
end;
inc.systemctl = function(p_name, p_cmd)
    return os.execute("/usr/bin/systemctl " .. p_cmd .. " " .. p_name)
end;
inc.monit = function()    
    if not inc.lsof("-t -i:" .. nsboot.cfg.dhcp.port) then
        inc.systemctl("isc-dhcp-server", "start");
        inc.systemctl("isc-dhcp-server", "restart");
    end
    if not inc.lsof("-t -i:" .. nsboot.cfg.tftp.port) then
        inc.systemctl("isc-dhcp-server", "start");
        inc.systemctl("tftpd-hpa", "restart");
    end
    if not inc.lsof("-t -i:" .. nsboot.cfg.iscsi.port) then
        inc.systemctl("isc-dhcp-server", "start");
        inc.systemctl("tgt", "restart");
    end
end;
inc.GetMacFromIPv4 = function(p_ipv4)
    if inc.checkconf() and p_ipv4 ~= nil then
        local i, v
...
inc.GetIDFromIPv4 = function(p_ipv4)
    if inc.checkconf() and p_ipv4 ~= nil then
        local i, v
        for i, v in pairs(nsboot.cfg.wks) do
            if p_ipv4 == v.ipv4 then
...
function inc.isFileOrDir(name)
    if type(name) ~= "string" then
        return false
    end
    return os.rename(name, name) and true or false
end
function inc.isSupperMode(id)
    if inc.checkconf() and tostring(nsboot.cfg.wks[tonumber(id)].supper) ~= nil and
...
function inc.ls_files(path)
    if inc.isDir(path) then
        local result, t_res = {}
